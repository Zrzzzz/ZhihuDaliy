...
// MARK: - UICollectionViewDataSource
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if scrollView.contentOffset.x > (CGFloat(imagePathsGroup.count - 1) * self.bounds.width) {
            mainView.scrollToItem(at: NSIndexPath.init(row: 0, section: 0) as IndexPath, at: UICollectionView.ScrollPosition.left, animated: false)
    ...
    @objc func automaticScroll(){
        ...
        if targetIndex == self.totalItemsCount {
            ...
            self.mainView.scrollToItem(at:NSIndexPath.init(row: 0, section: 0) as IndexPath, at: UICollectionView.ScrollPosition.left, animated: false)
            self.mainView.scrollToItem(at: NSIndexPath.init(row: 1, section: 0) as IndexPath, at: UICollectionView.ScrollPosition.left, animated: true)
            return
        }
        self.mainView.scrollToItem(at: NSIndexPath.init(row: targetIndex, section: 0) as IndexPath, at: UICollectionView.ScrollPosition.left, animated: true)
